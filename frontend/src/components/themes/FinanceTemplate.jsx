...
const FinanceTemplate = ({ data }) => {
  ...
  return (
    <div className="w-full break-before-auto break-after-auto bg-white text-black  relative px-8 py-10 shadow-lg font-serif">
      ...
      {projects.length > 0 && (
        <div className="mb-6">
          <h2 className="text-lg font-bold text-slate-800 border-b-2 border-slate-800 pb-1 mb-4">Projects</h2>
          <div className="space-y-5">
            {projects.map((proj) => (
              <div key={proj.id} className="break-inside-avoid mb-4">
                <div className="flex justify-between items-baseline mb-1">
                  ...
                  {proj.link && (
                    <a href={proj.link} target="_blank" rel="noopener noreferrer" className="text-sm text-slate-600 hover:underline italic">
                      View Project
                    </a>
                  )}
                </div>
                {proj.techStack && <p className="text-sm text-slate-500 italic mb-2">{proj.techStack}</p>}
                {proj.description ? (
                  <div className="text-sm text-slate-700">
                    {Array.isArray(proj.description) ? (
                      <ul className="list-disc ml-5 space-y-1.5 marker:text-gray-600 text-sm mt-2 text-slate-700">
